 ...
 * <p>
 * Verifica que las anotaciones de validación funcionan correctamente
 * para diferentes grupos de validación (Create, Update, PartialUpdate).
 */
...
class ValidationTest {
    ...
    // DTO de prueba con anotaciones de validación (las anotaciones en interfaces NO se heredan a records)
    record TestValidatedDTO(
            @jakarta.validation.constraints.Null(groups = ValidationGroups.Create.class, message = "ID debe ser nulo al crear una nueva entidad")
            @jakarta.validation.constraints.NotNull(groups = ValidationGroups.Update.class, message = "ID es requerido para actualizar")
            @jakarta.validation.constraints.Positive(message = "ID debe ser un número positivo")
            Long id,
            Boolean activo,
            String name
    ...
    class ComplexDTOValidationTests {
        ...
        record ComplexDTO(
                ...
                @jakarta.validation.constraints.Max(value = 150, message = "Age must be realistic")
                Integer age
        ) implements BaseDTOValidated {}
        ...
        void shouldValidateAllFieldsCorrectlyOnCreate() {
            ...
            ComplexDTO validDto = new ComplexDTO(
        ...
        void shouldCollectMultipleViolations() {
            ...
            ComplexDTO invalidDto = new ComplexDTO(
        ...
        void shouldValidateEmailFormat() {
            // Given
            ComplexDTO dtoWithInvalidEmail = new ComplexDTO(
                    null, true, "Valid Name", "not-an-email", 25
            );
        ...
        void shouldValidateNameLength() {
            ...
            ComplexDTO dtoWithShortName = new ComplexDTO(
                    null, true, "A", "test@email.com", 25
            );

            // When
            Set<ConstraintViolation<ComplexDTO>> violations =
                    validator.validate(dtoWithShortName);
