...
/**
 * Clase utilitaria con especificaciones JPA comunes para queries dinámicas.
 * Utiliza el patrón Specification de Spring Data JPA para construir consultas
 * de forma type-safe y componible.
 */
public final class BaseSpecification {
    ...
    public static <E extends Base> Specification<E> hasEstado(Boolean estado) {
        return (root, query, cb) -> estado != null
                ? cb.equal(root.get(FIELD_ESTADO), estado)
                : cb.conjunction();
    }
    ...
    public static <E extends Base> Specification<E> createdBefore(LocalDateTime date) {
        return (root, query, cb) -> date != null
                ? cb.lessThan(root.get(FIELD_FECHA_CREACION), date)
                : cb.conjunction();
    }
     ...
     */
    public static <E extends Base> Specification<E> createdBetween(LocalDateTime start, LocalDateTime end) {
     ...
     */
    public static <E extends Base> Specification<E> updatedBetween(LocalDateTime start, LocalDateTime end) {
        return (root, query, cb) -> {
            if (start != null && end != null) {
    ...
    public static <E extends Base> Specification<E> createdBy(String usuario) {
        return (root, query, cb) -> usuario != null
                ? cb.equal(root.get(FIELD_CREADO_POR), usuario)
                : cb.conjunction();
    }
