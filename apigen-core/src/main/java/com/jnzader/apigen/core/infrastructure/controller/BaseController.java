...
/**
 * Interfaz base para controladores REST genéricos. Define los endpoints estándar RESTful para
 * operaciones CRUD sobre DTOs.
 *
 * <p>Endpoints: - GET / - Listar con paginación y filtrado - HEAD / - Obtener conteo (X-Total-Count
 * header) - GET /{id} - Obtener por ID (soporta ETag) - HEAD /{id} - Verificar existencia - POST /
 * - Crear (retorna Location header) - PUT /{id} - Actualizar completo (valida ID, soporta If-Match)
 * - PATCH /{id} - Actualizar parcial - DELETE /{id} - Eliminar (soft delete por defecto) - DELETE
 * /{id}?permanent=true - Eliminar permanente
 *
...
public interface BaseController<D extends BaseDTO, I extends Serializable> {
    ...
    /** Obtiene el conteo de recursos. HEAD / → X-Total-Count header */
    ResponseEntity<Void> count(@RequestParam(required = false) Map<String, String> filters);

    /**
     * Obtiene un recurso por ID con soporte para ETag. GET /{id}
     *
    ...
    /** Verifica existencia de un recurso. HEAD /{id} → 200 si existe, 404 si no */
    ResponseEntity<Void> existsById(@PathVariable I id);

    /** Crea un nuevo recurso. POST / → 201 Created + Location header */
    ResponseEntity<?> save(@Valid @RequestBody D dto);

    /**
     * Actualiza completamente un recurso con validación de ID y concurrencia optimista. PUT /{id}
     *
    ...
    /** Actualiza parcialmente un recurso. PATCH /{id} */
    ...
    /**
     * Elimina un recurso (soft delete por defecto, hard delete con ?permanent=true). DELETE /{id}
    ...
    /** Restaura un recurso eliminado con soft delete. POST /{id}/restore */
     ...
     *
     * <p>GET /cursor?size=20&sort=id&direction=DESC GET /cursor?cursor=eyJpZCI6MTAwfQ==&size=20
     *
