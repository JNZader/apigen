 ...
 * <p>
 * Características:
 * - Solo mide métodos anotados con @Measured (selectivo)
 * - Mide automáticamente solo operaciones críticas de controlador público
 * - Registra métricas en Micrometer para exposición via Prometheus
 * - Loguea métodos lentos (> umbral configurable)
 * - Bajo overhead en operaciones no instrumentadas
 * <p>
 * Para medir un método específico, usar:
 * <pre>
 * {@code
 ...
 * </pre>
 * <p>
 * Only loads when MeterRegistry is available (i.e., Actuator is configured).
 */
@Aspect
@Component
...
public class MetricsAspect {
    ...
    /**
     * Pointcut para endpoints públicos de controlador (solo @GetMapping, @PostMapping, etc.).
     * Se miden automáticamente porque son el punto de entrada del usuario.
     */
    @Pointcut("execution(public * com.jnzader.apigen.core.controller..*Controller.*(..))")
    public void controllerPublicMethod() {}

    /**
     * Pointcut para operaciones de escritura de servicio.
     * Se miden automáticamente porque son operaciones críticas.
     */
    ...
    public Object measureAnnotatedMethod(ProceedingJoinPoint joinPoint, Measured measured) throws Throwable {
        if (!metricsEnabled) {
            return joinPoint.proceed();
        }

        String metricName = getMetricName(joinPoint, measured);
        long threshold = measured.slowThresholdMs() > 0 ? measured.slowThresholdMs() : slowThresholdMs;
    ...
    public Object measureClassMethod(ProceedingJoinPoint joinPoint, Measured measured) throws Throwable {
        if (!metricsEnabled) {
            return joinPoint.proceed();
        }
    ...
    /**
     * Mide endpoints de controlador automáticamente.
     * Solo endpoints públicos, excluyendo métodos ya medidos por @Measured.
     */
    ...
    private Object executeWithMetrics(ProceedingJoinPoint joinPoint, String metricName,
                                      String layer, boolean histogram, long threshold) throws Throwable {
