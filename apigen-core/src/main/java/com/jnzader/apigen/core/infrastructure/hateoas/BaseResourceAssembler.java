 ...
 * <p>
 * Convierte DTOs en EntityModel con links de navegación.
 * <p>
 * Links generados:
 * - self: enlace al recurso actual
 * - collection: enlace a la colección de recursos
 * - update: enlace para actualizar (PUT)
 * - delete: enlace para eliminar (DELETE)
 *
...
public abstract class BaseResourceAssembler<D extends BaseDTO, I extends Serializable>
    ...
    public EntityModel<D> toModel(D dto) {
        ...
        // Collection link
        model.add(linkTo(methodOn(controllerClass).findAll(null, null, null, Pageable.unpaged())).withRel("collection"));
    ...
    public CollectionModel<EntityModel<D>> toCollectionModel(Iterable<? extends D> entities) {
        ...
        collectionModel.add(linkTo(methodOn(controllerClass).findAll(null, null, null, Pageable.unpaged())).withSelfRel());
    ...
    public PagedModel<EntityModel<D>> toPagedModel(Page<D> page) {
        ...
        // Self link
        pagedModel.add(linkTo(methodOn(controllerClass).findAll(null, null, null, page.getPageable())).withSelfRel());

        // Links de navegación
        if (page.hasNext()) {
            pagedModel.add(Link.of(buildPageUrl(page.getNumber() + 1, page.getSize())).withRel("next"));
        }
        if (page.hasPrevious()) {
            pagedModel.add(Link.of(buildPageUrl(page.getNumber() - 1, page.getSize())).withRel("prev"));
        }
        pagedModel.add(Link.of(buildPageUrl(0, page.getSize())).withRel("first"));
        if (page.getTotalPages() > 0) {
            pagedModel.add(Link.of(buildPageUrl(page.getTotalPages() - 1, page.getSize())).withRel("last"));
        }
    ...
    /**
     * Construye una URL de paginación.
     * Las subclases pueden sobrescribir este método para personalizar la URL.
     */
    ...
    /**
     * Extrae el ID del DTO de forma segura.
     * Las subclases pueden sobrescribir si necesitan un tipo de ID diferente.
     *
    ...
    protected I extractId(D dto) {
        Long id = dto.id();
        if (id == null) {
            throw new IllegalArgumentException("El DTO debe tener un ID válido para generar links HATEOAS");
        }
        return (I) id;
    }
}
