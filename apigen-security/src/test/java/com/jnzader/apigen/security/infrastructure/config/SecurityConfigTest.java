 ...
 *
 * <p>Verifica: - Acceso a endpoints públicos - Protección de endpoints privados - CORS configurado
 * correctamente - CSRF configurado correctamente
 *
 * <p>Ejecutar con: ./gradlew test -Ptest.security
 */
@SpringBootTest
...
class SecurityConfigTest {
                            ...
                            result -> {
                                ...
                                if (status == 401 || status == 403) {
                                    throw new AssertionError(
                                            "Expected info endpoint to be accessible, but got "
                                                    + status);
                                }
                            });
        }
    ...
    class AuthenticationTypesTests {
        ...
        @Test
        @DisplayName(
                "should accept requests with or without Authorization header when security"
                        + " disabled")
        void shouldAcceptRequestsWithAuthHeaderWhenSecurityDisabled() throws Exception {
            ...
            mockMvc.perform(get("/test-entities").header("Authorization", "Bearer any-token"))
                    .andExpect(status().isOk());
        }
