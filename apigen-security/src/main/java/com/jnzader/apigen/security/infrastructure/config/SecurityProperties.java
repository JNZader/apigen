 ...
 * <ul>
 *   <li><b>jwt</b>: JWT propio con HS256 (default)</li>
 *   <li><b>oauth2</b>: OAuth2 Resource Server para IdPs externos (Auth0, Keycloak, Azure AD)</li>
 * </ul>
 * <p>
...
public class SecurityProperties {
    ...
    /**
     * Feature flag para habilitar/deshabilitar seguridad.
     * - true: Endpoints protegidos según el modo configurado
     * - false: Todos los endpoints son públicos (permitAll)
     */
     ...
     * Falla rápido si la configuración requerida no está presente.
     */
    @PostConstruct
    public void validate() {
        if (enabled) {
            if (mode == AuthMode.JWT) {
                ...
                log.info("  - Refresh token expiration: {} minutos", jwt.getRefreshExpirationMinutes());
    ...
    private void validateJwtSecret() {
        if (jwt.getSecret() == null || jwt.getSecret().isBlank()) {
            throw new IllegalStateException(
                    ...
                    "Configure la variable de entorno JWT_SECRET o la propiedad apigen.security.jwt.secret"
            );
        }
    ...
    public static class JwtProperties {
        ...
        /**
         * Clave secreta para firmar tokens JWT (mínimo 256 bits para HS256).
         * REQUERIDO cuando seguridad está habilitada.
         */
        private String secret;

        /**
         * Tiempo de expiración del token de acceso en minutos.
         * Default: 15 minutos (más seguro que 60).
         */
        private int expirationMinutes = 15;

        /**
         * Tiempo de expiración del refresh token en minutos.
         * Default: 7 días (10080 minutos).
         */
    ...
    public static class OAuth2Properties {
        ...
        /**
         * URI del issuer del IdP (Auth0, Keycloak, Azure AD, etc.).
         * Usado para validar tokens y obtener JWKS.
         * Ejemplos:
        ...
        /**
         * Audience esperada en el token (aud claim).
         * Generalmente es el identificador de tu API.
         */
        private String audience;

        /**
         * Nombre del claim que contiene los roles/permisos.
         * Varía según el IdP:
         *   - Auth0: "permissions" o custom namespace
        ...
        /**
         * Prefijo para los roles extraídos del claim.
         * Por ejemplo, "ROLE_" convierte "admin" en "ROLE_admin".
         */
