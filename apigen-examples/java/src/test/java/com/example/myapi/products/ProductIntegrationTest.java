package com.example.myapi.products;
...
/**
 * Integration tests for Product entity.
...
class ProductIntegrationTest {
    ...
    @DisplayName("1. POST - Should create new Product")
    void shouldCreateNewProduct() throws Exception {
        ...
        ProductDTO created = jsonMapper.readValue(response, ProductDTO.class);
    ...
    @DisplayName("2. GET /{id} - Should find Product by ID")
    ...
    @DisplayName("3. GET - Should list all Product with pagination")
    ...
    @DisplayName("4. HEAD /{id} - Should check Product exists")
    ...
    @DisplayName("6. PUT /{id} - Should update Product")
    void shouldUpdateProduct() throws Exception {
        ...
        ProductDTO dto = jsonMapper.readValue(
                getResult.getResponse().getContentAsString(), ProductDTO.class);

        dto.setName("Updated name");
        dto.setDescription("Updated description");
        dto.setPrice(new java.math.BigDecimal("199.99"));
        dto.setStock(100);
        dto.setSku("Updated sku");
        dto.setImageUrl("Updated imageUrl");
    ...
    @DisplayName("7. PATCH /{id} - Should partial update Product")
    void shouldPartialUpdateProduct() throws Exception {
        assertThat(createdId).isNotNull();
    ...
    @DisplayName("8. DELETE /{id} - Should soft delete Product")
    ...
    @DisplayName("9. POST /{id}/restore - Should restore soft-deleted Product")
    ...
    @DisplayName("10. DELETE /{id}?permanent=true - Should permanently delete Product")
    void shouldPermanentlyDeleteProduct() throws Exception {
        assertThat(createdId).isNotNull();
    ...
    @DisplayName("11. GET /{id} - Should return 404 for non-existent Product")
    ...
    void shouldSupportCursorPagination() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            ProductDTO uniqueDto = ProductDTO.builder()
                        .activo(true)
                        .name("Test name" + "-" + java.util.UUID.randomUUID().toString().substring(0, 8) + "-" + i)
                        .description("Test description" + "-" + java.util.UUID.randomUUID().toString().substring(0, 8) + "-" + i)
                        .price(new java.math.BigDecimal("199.99"))
                        .stock(100)
                        .sku("Test sku" + "-" + java.util.UUID.randomUUID().toString().substring(0, 8) + "-" + i)
                        .imageUrl("Test imageUrl" + "-" + java.util.UUID.randomUUID().toString().substring(0, 8) + "-" + i)
                        .build();
            mockMvc.perform(post(BASE_URL)
                            .contentType(MediaType.APPLICATION_JSON)
