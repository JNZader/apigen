package com.example.myapi.reviews;
...
/**
 * Integration tests for Review entity.
...
class ReviewIntegrationTest {
    ...
    private ReviewDTO testDto;
    private static Long createdId;

    @BeforeEach
    void setUp() {
        testDto = ReviewDTO.builder()
                .activo(true)
                .rating(100)
                .comment("Test comment")
                .reviewDate(java.time.LocalDateTime.now())
                .build();
    }

    @Test
    @org.junit.jupiter.api.Order(1)
    @DisplayName("1. POST - Should create new Review")
    void shouldCreateNewReview() throws Exception {
        ...
        ReviewDTO created = jsonMapper.readValue(response, ReviewDTO.class);
    ...
    @DisplayName("2. GET /{id} - Should find Review by ID")
    ...
    @DisplayName("3. GET - Should list all Review with pagination")
    ...
    @DisplayName("4. HEAD /{id} - Should check Review exists")
    ...
    @DisplayName("6. PUT /{id} - Should update Review")
    void shouldUpdateReview() throws Exception {
        ...
        ReviewDTO dto = jsonMapper.readValue(
                getResult.getResponse().getContentAsString(), ReviewDTO.class);
    ...
    @DisplayName("7. PATCH /{id} - Should partial update Review")
    void shouldPartialUpdateReview() throws Exception {
        assertThat(createdId).isNotNull();
    ...
    @DisplayName("8. DELETE /{id} - Should soft delete Review")
    void shouldSoftDeleteReview() throws Exception {
        assertThat(createdId).isNotNull();
    ...
    @DisplayName("9. POST /{id}/restore - Should restore soft-deleted Review")
    ...
    @DisplayName("10. DELETE /{id}?permanent=true - Should permanently delete Review")
    void shouldPermanentlyDeleteReview() throws Exception {
        assertThat(createdId).isNotNull();
    ...
    @DisplayName("11. GET /{id} - Should return 404 for non-existent Review")
    ...
    void shouldSupportCursorPagination() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            ReviewDTO uniqueDto = ReviewDTO.builder()
                        .activo(true)
                        .rating(100)
                        .comment("Test comment" + "-" + java.util.UUID.randomUUID().toString().substring(0, 8) + "-" + i)
                        .reviewDate(java.time.LocalDateTime.now())
                        .build();
            mockMvc.perform(post(BASE_URL)
                            .contentType(MediaType.APPLICATION_JSON)
